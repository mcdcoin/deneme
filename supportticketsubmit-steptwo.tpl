...
<section id="page" class="backgroundf7f7f7">
	<div class="container">
		...
		<p>Ürünleriniz ve diğer işlemleriniz için destek bildirimi oluşturun.</p>
	</div>
</section>
<section id="page">
  <div class="container">
    ...
    <form method="post" action="{$smarty.server.PHP_SELF}?step=3" enctype="multipart/form-data" role="form">
        ...
        <div class="row">
            <div class="form-group col-sm-6">
                ...
                <input type="text" name="name" id="inputName" value="{if $loggedin}{$clientname}{else}{$name}{/if}" class="basic-input-control{if $loggedin} disabled{/if}"{if $loggedin} disabled="disabled"{/if} />
            </div>
            <div class="form-group col-sm-6">
                <label for="inputEmail">{$LANG.supportticketsclientemail}</label>
                <input type="email" name="email" id="inputEmail" value="{$email}" class="basic-input-control{if $loggedin} disabled{/if}"{if $loggedin} disabled="disabled"{/if} />
            </div>
        </div>
        <div class="row">
            <div class="form-group col-sm-12">
                <label for="inputSubject">{$LANG.supportticketsticketsubject}</label>
                <input type="text" name="subject" id="inputSubject" value="{$subject}" class="basic-input-control" />
            </div>
        </div>
        <div class="row">
            <div class="form-group {if $relatedservices}col-sm-4{else}col-sm-6{/if}">
                ...
                <select name="deptid" id="inputDepartment" class="basic-input-control" onchange="refreshCustomFields(this)">
            ...
            {if $relatedservices}
                <div class="form-group col-sm-4">
                    ...
                    <select name="relatedservice" id="inputRelatedService" class="basic-input-control">
            ...
            <div class="form-group {if $relatedservices}col-sm-4{else}col-sm-6{/if}">
                ...
                <select name="urgency" id="inputPriority" class="basic-input-control">
        ...
        <div class="form-group">
            ...
            <textarea name="message" id="inputMessage" rows="12" class="basic-input-control markdown-editor" data-auto-save-name="client_ticket_open">{$message}</textarea>
        </div>
        ...
        <p class="text-center">
            <input type="submit" id="openTicketSubmit" value="{$LANG.supportticketsticketsubmit}" class="yesil-buton" />
            <a href="supporttickets.php" class="standartButon">{$LANG.cancel}</a>
        </p>
